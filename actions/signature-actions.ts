...
import type { Signature, SignatureWithPetition, SignatureFormData } from "@/types"
...
export async function createSignature(
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      amount: typeof signatureData.amount === "string" ? Number.parseFloat(signatureData.amount) : signatureData.amount,
      name: String(signatureData.isAnonymous).toLocaleLowerCase() === "true" ? "Anonymous" : signatureData.name,
 ...
 */
export async function listSignaturesForPetition(petitionId: string): Promise<Signature[]> {
  const supabase = await createClient()
